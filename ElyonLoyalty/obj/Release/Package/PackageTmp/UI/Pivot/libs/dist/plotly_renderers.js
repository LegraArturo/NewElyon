(function() {
  ...
  callWithJQuery(function($, Plotly) {
    ...
    return $.pivotUtilities.plotly_renderers = {
      "Horizontal Bar": makePlotlyChart({
        type: 'bar',
        orientation: 'h'
      }, {
        barmode: 'group'
      }, true),
      "Horizontal Apilado ": makePlotlyChart({
        type: 'bar',
        orientation: 'h'
      }, {
        barmode: 'relative'
      }, true),
      "Graf Barras": makePlotlyChart({
        type: 'bar'
      }, {
        barmode: 'group'
      }),
      "Graf Barras Apilado": makePlotlyChart({
        type: 'bar'
      }, {
        barmode: 'relative'
      }),
      "Graf Lineal": makePlotlyChart(),
      "Graf Area": makePlotlyChart({
        stackgroup: 1
      }),
      "Graf Dispersion": makePlotlyScatterChart(),
      'Graf Circular': makePlotlyChart({
        type: 'pie',
        scalegroup: 1,
